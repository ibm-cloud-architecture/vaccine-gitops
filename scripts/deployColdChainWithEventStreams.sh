...
KAFKA_BOOTSTRAP=$(oc get route -n ${EVENTSTREAMS_NS} ${KAFKA_CLUSTER_NAME}-kafka-bootstrap -o jsonpath="{.status.ingress[0].host}:443")
...
then
    ...
    oc apply -k  ${ENVPATH}/overlays -n ${EVENTSTREAMS_NS}
    sleep 5
...
oc get secret ${SCRAM_USER} -n ${EVENTSTREAMS_NS} -o json |  jq -r '.metadata.name="scram-user"' | jq -r '.metadata.namespace="'${YOUR_PROJECT_NAME}'"' | oc apply -f -

if [[ -z $(oc get secret ${TLS_USER}) ]]
then
...
# As the project is personal to the user, we can keep a generic name for the secret
oc get secret ${TLS_USER} -n ${EVENTSTREAMS_NS} -o json | jq -r '.metadata.name="tls-user"' | jq -r '.metadata.namespace="'${PROJECT_NAME}'"' | oc apply -f -
...
echo "When you are done with the lab do: \n oc delete -k apps/cold-chain"